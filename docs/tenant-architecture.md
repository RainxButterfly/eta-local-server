...
| 文件 | 说明 |
|---|---|
| `tenant/TenantContext.java` | ThreadLocal 租户上下文 |
| `tenant/TenantRoutingDataSource.java` | 动态数据源路由 |
| `tenant/DataSourceConfig.java` | 数据源 Bean 配置 |
| `tenant/UserDatabaseInitializer.java` | 用户数据库创建与注册 |
| `auth/UserAccount.java` | 用户登录凭证实体（默认库） |
| `auth/UserProfile.java` | 用户档案实体（用户库） |
| `auth/UserAccountRepository.java` | 默认库 Repository |
| `auth/UserProfileRepository.java` | 用户库 Repository |
| `resources/schema.sql` | 默认库建表脚本 |
| `resources/schema-user.sql` | 用户库建表脚本 |

### 修改文件

| 文件 | 改动 |
|---|---|
| `EtaApplication.java` | 无改动（@Primary 路由数据源使自动配置退避） |
| `application.yml` | 默认库改文件型 H2，ddl-auto: none，新增 user-db.path |
| `security/JwtAuthenticationFilter.java` | 认证后设置 TenantContext，ensureRegistered，finally 清除 |
| `auth/AuthService.java` | 重构为 UserAccount + UserProfile，去掉 @Transactional |
| `auth/AuthDtos.java` | UserVO.from 改为接收两个实体 |
| `auth/AuthController.java` | 无改动（接口兼容） |
| `config/DataInitializer.java` | 手动执行 schema.sql，创建 demo UserAccount + UserProfile |
| 6 个业务实体 | 删除 ownerId 字段 |
| 6 个业务 Repository | 删除 ownerId 相关查询方法 |
| 4 个业务 Service | 删除 ownerId 参数 |
| 4 个业务 Controller | 删除 EtaPrincipal 和 ownerId 传递 |

### 删除文件

| 文件 | 说明 |
|---|---|
| `auth/User.java` | 拆分为 UserAccount + UserProfile |
| `auth/UserRepository.java` | 拆分为两个 Repository |
